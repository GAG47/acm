...
**PMT**（Partial Match Table，**部分匹配表**）是一种数据表，也被称为**前缀函数**。对于一个字符串而言，既是它的前缀又是它的后缀的字符串称为**border**，pmt则表示 $s[1,...,i-1,i]$ 的最大border。举个例子： **s** 是文本串，**p** 是模式串，则 **PMT$[ i ]$** 表示从 **p[0]** 开始往后数，从 **p[i]** 开始往前数，在保证二者始终相同的情况下的最大步值。
